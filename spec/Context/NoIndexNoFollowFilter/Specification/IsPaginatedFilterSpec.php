...
class IsPaginatedFilterSpec extends ObjectBehavior
{
    ...
    function it_return_false_when_request_have_no_page(Request $request, ParameterBag $query)
    {
        $query->get('page', '')->shouldBeCalled()->willReturn('');
        $request->query = $query;
        $this->isSatisfiedBy($request)->shouldReturn(false);
    }

    function it_return_true_when_request_have_page(Request $request, ParameterBag $query)
    {
        $query->get('page', '')->shouldBeCalled()->willReturn(1);
        $request->query = $query;
