...
class OrSpecificationSpec extends ObjectBehavior
{
    ...
    function it_should_return_true_when_at_least_one_specification_return_true(
        Request $request,
        NoIndexNoFollowFilterInterface $impl1,
        NoIndexNoFollowFilterInterface $impl2,
        NoIndexNoFollowFilterInterface $impl3): void
    {
        $impl1->isSatisfiedBy($request)->willReturn(false);
        $impl2->isSatisfiedBy($request)->willReturn(true);
        $impl3->isSatisfiedBy($request)->willReturn(false);
    ...
    {
        $trueImpl->isSatisfiedBy($request)->willReturn(true);
